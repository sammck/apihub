...
hashed_password = HashedPassword(f'{resource_prefix}apihub-ec2_user_hashed_password', ec2_user_password)
...
ebs_data_vol = EbsVolume(resource_prefix=f'{resource_prefix}apihub-', volid=data_vol_id, use_config=False)
...
main_dns_zone_id = cast(str, require_stack_output(f'{aws_env_import_prefix}parent_dns_zone_id', stack=aws_env_stack_name))
assert isinstance(main_dns_zone_id, str)
...
ec2_instance = Ec2Instance(
    vpc=vpc,
    az=az,
    resource_prefix=f"{resource_prefix}apihub-",
    use_config=True,
    cfg_prefix=f"{cfg_prefix}apihub_",
...
cloud_config_obj: Input[JsonableDict] = dict(
    ...
    users = [
          ...
          },
        {
            ...
            # we are mounting over /home, any home directory created here will be
            # obliterated by our mount. This includes the SSH authorized_keys
            # file. We will work around this by creating the home dir in our init script...
            'no-create-home': True,
          },
      ],
    device_aliases = dict(
...
pulumi.export(f'{export_prefix}apihub_cloud_init_succeeded', cloud_init_succeeded)
...
pulumi.export(f"{export_prefix}cached_host_key_log", cached_host_key.cmd_out)
...
#      }
ec2_instance.stack_export(export_prefix=export_prefix)
